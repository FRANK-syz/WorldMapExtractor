...
`WorldMapExtractor` 是一个可交互的 Python GUI 工具，能够自动识别世界地图图片，根据图片提供颜色条（Colorbar）的类型，将论文/文献中的地图图片还原为可用于科研的、具有实际数值的地理栅格数据（GeoTIFF）。当在文献中遇到关键地图数据并未公开提供下载时，本工具提供了一种"以图还原数据"的可行方案。

> 华东师范大学 地理科学学院 研究生课程论文《世界地图提取器——基于图像特征识别的图片转地理栅格文件工具》
...
> 提示：完整打包客户端为 `WorldMapExtractor.exe`（约 372 MB，独立可运行），因体积超出 GitHub 单文件 100MB 上传上限，未包含在本仓库中。
